...
export default async function InventoryPage({searchParams}:{searchParams:Promise<{saved?:string,error?:string,count?:string}>}){
  ...
  return <main className="admin-dashboard-shell">
    ...
    <section className="section" style={{paddingTop:42}}><div className="container">
      ...
      <div className="section-head"><div><div className="eyebrow">Stock control</div><h1>Low-stock inventory alerts</h1><p className="muted" style={{maxWidth:760}}>Each active package size has its own stock quantity and reorder threshold. Update one item at a time or use Bulk Restock to save many quantities together.</p></div><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><Link className="btn btn-outline" href="/admin/inventory/history">View stock history</Link><Link className="btn btn-outline" href="/admin/products">Manage full catalogue</Link></div></div>
