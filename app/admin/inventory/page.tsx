...
function suggestedReorder(stock:number,threshold:number){return stock<=threshold?Math.max((threshold*2)-stock,0):0}
...
export default async function InventoryPage({searchParams}:{searchParams:Promise<{saved?:string,error?:string,count?:string}>}){
  ...
  const stockValue=active.reduce((sum:number,v:any)=>{const p:any=productMap.get(v.product_id);return sum+(Number(v.stock_quantity||0)*Number(v.price??p?.price??0))},0)
  const reorderUnits=alerts.reduce((sum:number,v:any)=>sum+suggestedReorder(Number(v.stock_quantity||0),Number(v.reorder_threshold??5)),0)
  return <main className="admin-dashboard-shell">
    ...
    <section className="section" style={{paddingTop:42}}><div className="container">
      ...
      <div className="section-head"><div><div className="eyebrow">Stock control</div><h1>Low-stock inventory alerts</h1><p className="muted" style={{maxWidth:760}}>Each active package size has its own stock quantity and reorder threshold. Update one item at a time or use Bulk Restock to save many quantities together.</p></div><div style={{display:'flex',gap:10,flexWrap:'wrap'}}><a className="btn btn-primary" href="/admin/inventory/export">Export inventory CSV</a><Link className="btn btn-outline" href="/admin/inventory/history">View stock history</Link><Link className="btn btn-outline" href="/admin/products">Manage full catalogue</Link></div></div>
      <div className="admin-dashboard-stats" style={{marginTop:22}}><article className="admin-dashboard-stat"><span>Needs attention</span><strong>{alerts.length}</strong><p>Variants at or below threshold</p></article><article className="admin-dashboard-stat"><span>Out of stock</span><strong>{outOfStock}</strong><p>Active variants with zero units</p></article><article className="admin-dashboard-stat"><span>Healthy stock</span><strong>{healthy}</strong><p>Active variants above threshold</p></article><article className="admin-dashboard-stat"><span>Current stock value</span><strong>{money(stockValue)}</strong><p>Retail value of units currently recorded</p></article><article className="admin-dashboard-stat"><span>Suggested reorder</span><strong>{reorderUnits}</strong><p>Units needed to reach roughly 2× each threshold</p></article></div>
      ...
      {alerts.length?<section className="admin-dashboard-panel" style={{marginTop:28,border:'2px solid var(--burgundy)'}}><div className="section-head" style={{marginBottom:18}}><div><div className="eyebrow">Bulk restock</div><h2>Update all priority stock in one save</h2><p className="muted" style={{marginBottom:0}}>Enter the physical quantity available for each item below, then click Save all stock updates once. The suggested reorder quantity is a planning guide that brings stock to about twice its reorder threshold.</p></div></div><form action={bulkUpdateStock}><div className="admin-dashboard-table-wrap"><table className="admin-dashboard-table"><thead><tr><th>Product</th><th>Size</th><th>Current</th><th>Reorder at</th><th>Suggested reorder</th><th>New stock quantity</th></tr></thead><tbody>{alerts.map((v:any)=>{const p:any=productMap.get(v.product_id);const stock=Number(v.stock_quantity||0);const threshold=Number(v.reorder_threshold??5);return <tr key={v.id}><td><strong>{p?.name||'Product'}</strong><input type="hidden" name="bulk_variant_id" value={v.id}/></td><td>{v.size_label}</td><td>{stock}</td><td>{threshold}</td><td><strong>{suggestedReorder(stock,threshold)}</strong></td><td><input name="bulk_stock_quantity" type="number" min="0" defaultValue={stock} style={{width:110}} required/></td></tr>})}</tbody></table></div><div style={{display:'flex',justifyContent:'flex-end',marginTop:18}}><button className="btn btn-primary">Save all stock updates</button></div></form></section>:null}
