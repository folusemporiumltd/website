...
export default async function InventoryPage({searchParams}:{searchParams:Promise<{saved?:string,error?:string}>}){
  ...
  return <main className="admin-dashboard-shell">
    ...
    <section className="section" style={{paddingTop:42}}><div className="container">
      ...
      <div className="section-head"><div><div className="eyebrow">Stock control</div><h1>Low-stock inventory alerts</h1><p className="muted" style={{maxWidth:760}}>Each active package size has its own stock quantity and reorder threshold. Update both directly here without opening each product separately.</p></div><Link className="btn btn-outline" href="/admin/products">Manage full catalogue</Link></div>
      ...
      {params.saved?<p style={{padding:12,borderRadius:10,background:'#f2f8f2',marginTop:20}}>{params.saved==='stock'?'Stock quantity updated successfully.':'Reorder threshold updated successfully.'}</p>:null}
