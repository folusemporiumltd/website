...
export default async function ShopPage({ searchParams }: { searchParams: Promise<{ category?: string; search?: string }> }) {
  ...
  const categoryProducts = selectedCategory ? products.filter(p => p.category_id === selectedCategory.id) : products
  const normalizedSearch = search?.trim().toLowerCase()
  const filteredProducts = normalizedSearch ? categoryProducts.filter(p => `${p.name} ${p.description ?? ''}`.toLowerCase().includes(normalizedSearch)) : categoryProducts
