...
export default async function Home({ searchParams }: { searchParams: Promise<{ code?: string }> }) {
  ...
  return (
    <main>
      ...
      <section className="section"><div className="container"><div className="section-head"><div><div className="eyebrow">Featured</div><h2>Our product collection</h2></div><Link className="btn btn-outline" href="/shop">View all products</Link></div>{products?.length ? <div className="products">{products.map((p) => { const variant = defaultVariantByProduct.get(p.id); return <article className="product-card" key={p.id}><Link href={`/shop/${p.slug}`} aria-label={`View ${p.name}`}><div className="product-image">{p.image_url ? <img src={p.image_url} alt={p.name} style={{width:'100%',height:'100%',objectFit:'contain'}}/> : <span>{p.name}</span>}</div></Link><div className="product-body"><span className="status">{p.featured ? 'Featured' : 'Folus collection'} · {variant?.size_label ?? '500g'}</span><h3><Link href={`/shop/${p.slug}`}>{p.name}</Link></h3><p>{p.description}</p><strong>{variant && Number(variant.price) > 0 ? `₦${Number(variant.price).toLocaleString('en-NG')}` : Number(p.price) > 0 ? `₦${Number(p.price).toLocaleString('en-NG')}` : 'Price to be updated'}</strong><ProductCardActions product={p} variant={variant}/></div></article> })}</div> : <div className="empty"><h3>Our collection is being prepared.</h3><p>Product catalogue setup is underway. Check the Shop page as we continue adding the collection.</p><Link className="btn btn-primary" href="/shop">Visit Shop</Link></div>}</div></section>
      ...
      <footer className="footer luxury-footer">
        <div className="container footer-links-grid">
          ...
          <div className="footer-contact"><h3>Contact Information</h3><a href="mailto:folusemporium@gmail.com">folusemporium@gmail.com</a><a href="mailto:info@folusemporium.com">info@folusemporium.com</a><a href="tel:+2349168157255">+234 916 815 7255</a><a href="https://wa.me/2349168157255">WhatsApp: +234 916 815 7255</a><p>Alarere, Ibadan, Oyo State</p></div>
        </div>
        <div className="container footer-social-row"><div><b>Follow Folus Emporium</b><div className="footer-socials"><a href="https://www.facebook.com/folusemporiumltd" aria-label="Facebook">f</a><span aria-label="Instagram">◎</span><a href="https://www.tiktok.com/@folusemporium25" aria-label="TikTok">♪</a><span aria-label="YouTube">▶</span><span aria-label="LinkedIn">in</span></div></div><div className="payment-icons"><span>Paystack</span><span>Flutterwave</span><span>VISA</span><span>Mastercard</span></div></div>
