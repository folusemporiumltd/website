...
const storefrontFallback = {
  ...
  social: { facebook: 'https://www.facebook.com/folusemporiumltd', instagram: '', tiktok: 'https://www.tiktok.com/@folusemporium25', youtube: '', linkedin: '', whatsapp: 'https://wa.me/2349168157255' },
...
export default async function Home({ searchParams }: { searchParams: Promise<{ code?: string }> }) {
  ...
  for (const variant of variants ?? []) { const existing = defaultVariantByProduct.get(variant.product_id); if (!existing || Math.abs(variant.size_grams - 500) < Math.abs(existing.size_grams - 500)) defaultVariantByProduct.set(variant.product_id, variant) }
  return <main>
    ...
    <footer className="footer luxury-footer"><div className="container footer-links-grid">
    ...
    </div><div className="container footer-social-row"><div><b>Follow Folus Emporium</b><div className="footer-socials">{social.facebook?<a href={social.facebook} target="_blank" rel="noreferrer" aria-label="Facebook">f</a>:null}{social.instagram?<a href={social.instagram} target="_blank" rel="noreferrer" aria-label="Instagram">◎</a>:null}{social.tiktok?<a href={social.tiktok} target="_blank" rel="noreferrer" aria-label="TikTok">♪</a>:null}{social.youtube?<a href={social.youtube} target="_blank" rel="noreferrer" aria-label="YouTube">▶</a>:null}{social.linkedin?<a href={social.linkedin} target="_blank" rel="noreferrer" aria-label="LinkedIn">in</a>:null}</div></div><div className="footer-payment-logos" role="group" aria-label="Accepted payment methods" style={{display:'flex',alignItems:'center',flexWrap:'nowrap',gap:14,padding:'10px 12px',background:'#fffdf9',border:'1px solid #e2ad46',borderRadius:8,maxWidth:'100%',boxSizing:'border-box'}}><img src="/Paystack.png" alt="Paystack" width={116} height={28} style={{display:'block',width:116,height:28,objectFit:'cover',flexShrink:1,minWidth:0,mixBlendMode:'multiply'}}/><img src="/visa-and-mastercard-logo-26.png" alt="Visa and Mastercard" width={132} height={34} style={{display:'block',width:132,height:34,objectFit:'contain',flexShrink:1,minWidth:0,mixBlendMode:'multiply'}}/></div></div><div className="container copyright footer-bottom"><img src="https://boaaiskncrfmaismhqno.supabase.co/storage/v1/object/public/product-images/folus-emporium-horizontal-logo.png" alt="Folus Emporium"/><span>© 2026 Folus Emporium. All Rights Reserved. · Nature’s Goodness, Purely Yours.</span></div></footer>
  </main>
}
