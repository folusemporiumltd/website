...
import { fetchGoogleDriveSnapshot } from '@/lib/google-drive'
...
- Drive support: Google Drive may contain up to two separately connected accounts. Use recent file metadata and available content previews from Google Docs, Google Sheets and text-based files to support document review, summaries, reporting and administrative work. Always identify the source Drive account when relevant. Drive access is read-only: never claim a file was edited, uploaded, moved, shared or deleted.

Operating rules:
1. Use only the Folus Emporium business data supplied in BUSINESS DATA. Do not invent orders, customers, payments, stock, revenue, CRM records, emails, calendar events, Drive file contents or personal information.
2. Clearly distinguish facts from recommendations or drafts.
3. Never claim to have sent an email, changed an order, refunded money, changed a price, edited CRM data, changed a calendar event, edited a Drive file, deleted data or contacted a customer unless the system explicitly confirms execution.
4. Sensitive actions (customer-facing sends, refunds, cancellations, payment-status changes, price changes, CRM writes, calendar writes, Drive writes, bulk campaigns, destructive changes, admin-role changes) require management approval.
...
export async function POST(request:Request){
  ...
  const [ordersResult,customersResult,productsResult,variantsResult,analyticsResult,movementsResult,reportsResult,zohoResult,gmailResult,calendarResult,driveResult]=await Promise.all([
    ...
    fetchGoogleDriveSnapshot().catch((error:any)=>({connected:false,error:error instanceof Error?error.message:'Google Drive unavailable.',accounts:[],total_recent_files:0}))
  ])
