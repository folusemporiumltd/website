...
export async function GET(request: NextRequest) {
  ...
  const { count } = await admin.from('ai_agent_integrations').select('id',{count:'exact',head:true}).eq('provider','google_calendar')
  ...
  response.cookies.set('folus_google_calendar_oauth_state', state, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', path: '/', maxAge: 10 * 60 })
  return response
}
