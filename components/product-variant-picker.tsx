...
export default function ProductVariantPicker({ product, variants }: { product: Product; variants: Variant[] }) {
  ...
  return <div className="variant-picker">
    ...
    {added?<Link className="btn btn-primary" href="/cart">Proceed to Cart</Link>:<button className="btn btn-primary" onClick={add} disabled={!selected || selected.stock_quantity < quantity}>{selected?.stock_quantity ? 'Add to cart' : 'Out of stock'}</button>}
  </div>
}
